...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['year']:
            cur_year = options['year']
        else:
            n = datetime.now()
            cur_year = n.strftime("%Y")

        for f in forums:
            if options.get('archive', "0") == "1":
                links = parse_arc("%sarchive" % f.url, cur_year=cur_year)
